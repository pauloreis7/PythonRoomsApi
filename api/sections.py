...
async def find_section(
    ...
    check_session_exists = await get_section_by_id(section_id=section_id)
...
async def read_section_by_title(
    ...
    sections = await get_sections_by_title(sections_title=sections_title)
...
async def create_section(
    ...
    check_course_exists = await get_course_by_id(course_id=section.course_id)

    if check_course_exists is None:
        raise HTTPException(status_code=404, detail="Course not found")

    create_db_section_response = await create_db_section(section=section)
...
async def patch_section(
    ...
    check_section_exists = await get_section_by_id(section_id=section_id)

    if check_section_exists is None:
        raise HTTPException(status_code=404, detail="Course section not found")

    await patch_db_section(section_id=section_id, section=section)
...
async def delete_section(
    ...
    await delete_db_section(section_id=section_id)
